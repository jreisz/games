...
export class Board extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            boardCells: [],
            numberOfFlags: this.props.numberOfFlags,
    ...
    createBoard() {
        ...
        for (let i =0; i<this.props.boardWidth; i++){
            ...
            for (let j=0; j<this.props.boardHeight; j++){
                boardCells[i][j] = { mine: false, flag: false, value: 0, reveald: false, location: {width: i, height: j}};
            }
        }
        return boardCells;
    }
    ...
    shuffleMines(boardCells) {
        let minesArray = [], randomArray = [];
        for(let i=0; i< this.props.boardWidth; i++) { 
            for(let j=0; j<this.props.boardHeight; j++){ randomArray.push({width: i, height: j})}
        }
        randomArray = this.randomMines(randomArray);
        for (let i=0; i<this.props.boardMines; i++) {
            boardCells[randomArray[i].width][randomArray[i].height].mine = true;
            minesArray.push({width: randomArray[i].width ,height: randomArray[i].height});
        } 
        return minesArray;
    }
    ...
    randomMines(minesArrayForShuffle) {
        for (let i = minesArrayForShuffle.length - 1; i > 0; i--) {
            ...
            [minesArrayForShuffle[i], minesArrayForShuffle[j]] = [minesArrayForShuffle[j], minesArrayForShuffle[i]];
        }
    ...
    flagClicked(cellData) {
        if (this.props.boardUsedFlags === 0 && !cellData.flag){ alert('You dont have any more flags!') } 
        else {
            ...
            if ( !isCellFlagged && cellData.mine ) { flaggedMines--; }
            if ( isCellFlagged && cellData.mine ){ 
                flaggedMines++; 
                if (flaggedMines >= this.props.boardMines){
    ...
    updateBoardWithFlag(cellData){
        const isFlagged = !this.state.boardCells[cellData.location.width][cellData.location.height].flag;
        let updatedCellBoard = this.state.boardCells.map((arr) => arr.slice() );
        updatedCellBoard[cellData.location.width][cellData.location.height].flag = isFlagged;
        this.setState({boardCells: updatedCellBoard});
        return isFlagged;
    }

    componentDidUpdate(){
        if(this.props.newGame){ this.buildBoard(); }
    }
    ...
    revealCells(clickedCell){
        ...
        if (clickedCell.value > 0 ) {
            ...
            this.runOnNeighbors(clickedCell.location.width, clickedCell.location.height, newBoardCells, false);
        }
    ...
    render() {
        return (
            <div id="board-root">
                <table class='board' style={{overflowX:'auto', borderSpacing: '1px',  borderCollapse: 'separate'}}>
                    <tbody>
                        {this.state.boardCells.map((row, rowIndex) =>
                                <tr> {row.map((cell, cellIndex) =>
                                    <Cell data={cell} 
                                        key={cellIndex} 
                                        boardMeasures={{ containerHeight: this.state.containerHeight, containerWidth: this.state.containerWidth, numOfHeightCells: this.props.boardHeight, numOfWidthCells: this.props.boardWidth}} 
